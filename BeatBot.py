...
async def on_command_error(ctx, error):
    if isinstance(error, commands.CommandNotFound):
        nop_e = discord.Embed(
            ...
            title=f"RTFM!")
        nop_e.add_field(name="Apprendre à taper...", value=f"Si on m'avait filé un euro à\nchaque fois que tu te plantes,\nje serais riche.", inline=True)
...
async def request(ctx, feature):
    ...
    creator = await bot.fetch_user(265781552926031872)
...
async def on_message(message):  
    if "connard" in message.content:
        connard_answ =[
            'Oui?','C\'est mon deuxieme prénom','On m\'appelle?', 'Tu te trompes de personne, je ne suis pas <@589153146878820364>', 'C\'est toi le connard non mais!', 
            'Qui m\'invoque?', 'Pourquoi tu t\'appelles toi même ?', 'Tu m\'as pris pour <@411157117425156099> ou quoi ?']
